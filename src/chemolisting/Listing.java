...
public class Listing {
	...
	private int[][] model() {
		try {
			...
			// ** Constraints **
			// Number of treatments of each type should sum to total number of
			// treatments
			...
			// Sum of each treatment type's cases started per chair must equal
			// subtotalTreatments[k]
			...
			// No treatment can be scheduled to start after a certain time
			// (PREFIXED TO BE 6.00pm)
			for (int j = 0; j < NUM_TREATMENTS; j++) {
			...
			// Chairs occupied cannot exceed total chairs, cannot exceed a
			// certain manpower factor
			// Number of treatments started cannot exceed manpower available at
			// that time, cannot exceed a preset upper bound (to prevent
			// pharmacy overload)
			// Number of started cases per available nurse (not helping to
			// start) must be less than a certain ratio
			for (int i = 0; i < NUM_SLOTS; i++) {
